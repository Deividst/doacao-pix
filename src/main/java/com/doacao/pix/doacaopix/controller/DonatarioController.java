...
public class DonatarioController {
    ...
    @PostMapping
    public ResponseEntity<DonatarioDto> save(@RequestBody DonatarioDto donatarioDto) {
        return ResponseEntity.ok()
                .body(this.donatarioService.salvar(donatarioDto));
    }

    @PutMapping
    public ResponseEntity<DonatarioDto> update(@RequestBody DonatarioDto donatarioDto) {
        return ResponseEntity.ok()
    ...
    public ResponseEntity<DonatarioDto> findById(@PathVariable Long codigo) {
        DonatarioDto donatarioDto = this.donatarioService.buscarPorCodigo(codigo);
        return donatarioDto != null ? ResponseEntity.ok(donatarioDto) : ResponseEntity.notFound().build();
    }
}
